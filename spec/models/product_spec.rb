...
RSpec.describe Product, type: :model do
  describe 'Validations' do
    ...
    it "raise an exception for the name attribute if the product is saved without a name" do
      @product = @category.products.create(name: nil, price: 2500, quantity: 38, category: @category)

      expect(@product.errors.full_messages).to include("Name can't be blank")
    end

    it "raise an exception for the price attribute if the product is saved without a price" do
      @product = @category.products.create(name: "Prickly Pear", quantity: 38, category: @category)

      expect(@product.errors.full_messages).to include("Price can't be blank")
    end

    it "raise an exception for the quantity attribute if the product is saved without a quantity" do
      @product = @category.products.create(name: "Prickly Pear", price: 2500, quantity: nil, category: @category)

      expect(@product.errors.full_messages).to include("Quantity can't be blank")
    end

    it "raise an exception for the category attribute if the product is saved without a category" do
